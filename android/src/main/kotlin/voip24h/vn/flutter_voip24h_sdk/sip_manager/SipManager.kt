package voip24h.vn.flutter_voip24h_sdk.sip_manager

import android.content.Context
import android.util.Log
import io.flutter.plugin.common.EventChannel
import org.linphone.core.*
import voip24h.vn.flutter_voip24h_sdk.model.SipConfiguration
import io.flutter.plugin.common.MethodChannel.Result
import voip24h.vn.flutter_voip24h_sdk.FlutterVoip24hSdkPlugin
import voip24h.vn.flutter_voip24h_sdk.utils.*
...
internal class SipManager private constructor(context: Context) {
    ...
    private val coreListener = object : CoreListenerStub() {
        ...
        override fun onAccountRegistrationStateChanged(
            ...
            val param = createParams(EventAccountRegistrationStateChanged, "registrationState" to (state?.name ?: ""), "message" to message)
            FlutterVoip24hSdkPlugin.eventSink?.success(param)
        }
        ...
        override fun onCallStateChanged(
            ...
            when (state) {
                Call.State.IncomingReceived -> {
                    ...
                    val data = createParams(EventRing, "extension" to extension, "phoneNumber" to phoneNumber, "callType" to CallType.inbound.value)
                ...
                Call.State.OutgoingProgress -> {
                    ...
                    val extension = core.defaultAccount?.contactAddress?.username ?: ""
                    val phoneNumber = call.remoteAddress.username ?: ""
                    val data = createParams(EventRing, "extension" to extension, "phoneNumber" to phoneNumber, "callType" to CallType.outbound.value)
                    FlutterVoip24hSdkPlugin.eventSink?.success(data)
                }
                ...
                Call.State.StreamsRunning -> {
                    ...
                    val data  = createParams(EventUp, "callId" to callId)
                    FlutterVoip24hSdkPlugin.eventSink?.success(data)
                }
                Call.State.Paused -> {
                    Log.d(TAG, "Paused")
                    isPause = true
                    val data = createParams(EventPaused)
                    FlutterVoip24hSdkPlugin.eventSink?.success(data)
                }
                Call.State.Resuming -> {
                    Log.d(TAG, "Resuming")
                    val data = createParams(EventResuming)
                    FlutterVoip24hSdkPlugin.eventSink?.success(data)
                }
                ...
                Call.State.Released -> {
                    if(isMissed(call.callLog)) {
                        ...
                        val data = createParams(EventMissed, "phoneNumber" to callee, "totalMissed" to totalMissed)
                        FlutterVoip24hSdkPlugin.eventSink?.success(data)
                    } else {
                        Log.d(TAG, "Released")
                ...
                Call.State.End -> {
                    ...
                    val data = createParams(EventHangup, "duration" to duration)
                    FlutterVoip24hSdkPlugin.eventSink?.success(data)
                    timeStartStreamingRunning = 0
                }
                Call.State.Error -> {
                    Log.d(TAG, "Error")
                    val data = createParams(EventError, "message" to message)
                    FlutterVoip24hSdkPlugin.eventSink?.success(data)
                }
                else -> {
